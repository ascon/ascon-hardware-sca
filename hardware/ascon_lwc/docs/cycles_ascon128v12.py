...
CYCLES_P = 2 # cycles per permutation round
...
cycles_enc += PA*CYCLES_P                                   # init process
cycles_enc += 1                                             # init key add
cycles_enc += (PB*CYCLES_P+R//CCW)*Na                       # absorb + process complete AD blocks
cycles_enc += Ina*(int(((Bla*8+CCW-8)/CCW)) + PB*CYCLES_P)  # absorb + process incomplete AD blocks
cycles_enc += ((Na>0)|(Nm>0)|(Inm>0))*(Ina==0)              # absorb empty AD block with padding if required
cycles_enc += 1                                             # domain seperation
cycles_enc += (PB*CYCLES_P+R//CCW)*Nm                       # absorb + process complete MSG blocks
cycles_enc += Inm*(int(((Blm*8+CCW-8)/CCW)) + PB*CYCLES_P)  # absorb + process incomplete MSG blocks
cycles_enc += (Inm==0)                                      # absorb empty MSG block with pad
cycles_enc += 1                                             # final key add 1
cycles_enc += PA*CYCLES_P                                   # final process
...
cycles_dec += PA*CYCLES_P                                   # init process
cycles_dec += 1                                             # init key add
cycles_dec += (PB*CYCLES_P+R//CCW)*Na                       # absorb + process complete AD blocks
cycles_dec += Ina*(int(((Bla*8+CCW-8)/CCW)) + PB*CYCLES_P)  # absorb + process incomplete AD blocks
cycles_dec += ((Na>0)|(Nc>0)|(Inc>0))*(Ina==0)              # absorb empty AD block with padding if required
cycles_dec += 1                                             # domain seperation
cycles_dec += (PB*CYCLES_P+R//CCW)*Nc                       # absorb + process complete MSG blocks
cycles_dec += Inc*(int(((Blm*8+CCW-8)/CCW)) + PB*CYCLES_P)  # absorb + process incomplete MSG blocks
cycles_dec += (Inc==0)                                      # absorb empty MSG block with pad
cycles_dec += 1                                             # final key add 1
cycles_dec += PA*CYCLES_P                                   # final process
